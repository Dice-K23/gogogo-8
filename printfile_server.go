...
// アクセスがあったとき、アップロードフォームを返す --- (*2)
func indexHandler(w http.ResponseWriter, r *http.Request) {
	s := "<html><body>" + // ←別のHTMLファイルから呼び出せないのか？
		"<h1>ファイルを指定してください</h1>" +
...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
	file, _, err := r.FormFile("upfile") // ファイルを取得。indexHandlerの"<input type='file' name='upfile'>"
	if err != nil {
		w.Write([]byte("UPLOAD ERROR"))
		return
	}
	data, err := ioutil.ReadAll(file) // ファイルを読みだす
	if err != nil {
		w.Write([]byte("UPLOAD ERROR"))
		return
	}
